...
function FloatingTagBar({
  ...
  return (
    <div className="relative w-full overflow-hidden rounded-[1.2rem] border border-white/12 bg-white/8 px-3 py-3 sm:px-4">
      ...
      <div className={`flex w-max items-center gap-2 pl-4 pr-5 sm:gap-2.5 sm:pl-5 sm:pr-7 ${animationClassName}`}>
    ...
    <article className="overflow-hidden rounded-[1.8rem] border border-white/12 bg-white/8 shadow-[0_24px_58px_-40px_rgba(0,0,0,0.26)]">
      <button type="button" className="block w-full text-left" onClick={onToggle}>
        ...
        <div className="border-t border-white/10 px-5 py-5 sm:px-6">
          <p className="text-[0.82rem] leading-[1.75] text-cream-50/78 sm:text-[0.88rem]">
            {isExpanded ? story : `${story.slice(0, 110)}...`}
          </p>
          <span className="mt-4 inline-flex text-[0.68rem] font-semibold uppercase tracking-[0.16em] text-gold-400">
                ...
                <div className="mt-4 grid grid-cols-3 gap-3">
                  {introStats.map((item) => (
                    <div key={item.label} className="rounded-[1rem] border border-white/12 bg-white/8 px-3 py-3">
                      ...
                      <p className="mt-1.5 text-[0.58rem] uppercase tracking-[0.18em] text-cream-50/54 sm:text-[0.62rem]">
                        {item.label}
                      </p>
                    </div>
                  ))}
          ...
          <div className="mx-auto max-w-6xl">
            ...
            <div className="mt-6 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
              {operationsHighlights.map((item) => {
                ...
                return (
                  <SectionReveal key={item.id} className="overflow-hidden rounded-[1.45rem] border border-[#ddd2c1] bg-white/82 text-left shadow-[0_20px_48px_-42px_rgba(11,61,46,0.22)]">
                    <div className="relative">
                      <img
                        src={item.image}
                        alt={item.title}
                      ...
                      <div className="absolute inset-x-0 bottom-0 p-4">
                        <h3 className="font-display text-[1.15rem] leading-[1] text-cream-50 sm:text-[1.22rem]">{item.title}</h3>
                      </div>
                    </div>
                    <div className="p-4 sm:p-5">
                      <p className="text-[0.78rem] leading-relaxed text-forest-900/72 sm:text-[0.82rem]">{item.body}</p>
                    </div>
                  </SectionReveal>
                )
              })}
              ...
              {leadershipCards.map((card) => (
                <SectionReveal key={card.id}>
                  <LeadershipCard
                    ...
                    story={card.story}
                    isExpanded={expandedLeader === card.id}
                    onToggle={() => setExpandedLeader((current) => (current === card.id ? null : card.id))}
                  />
                </SectionReveal>
              ))}
            </div>
          ...
          <div className="mx-auto grid w-full max-w-[54rem] gap-4">
            ...
            <SectionReveal className="w-full overflow-hidden rounded-[1.7rem] border border-white/12 bg-white/8 p-5 shadow-[0_24px_54px_-42px_rgba(0,0,0,0.2)] sm:p-6 lg:p-7">
              ...
              <div className="mt-5 flex flex-wrap gap-2.5 sm:gap-3">
                <span className="rounded-full border border-white/12 bg-white/8 px-3 py-1.5 text-[0.74rem] font-medium text-cream-50/76 sm:px-3.5 sm:py-2 sm:text-[0.88rem]">
                  Coming Soon
                </span>
              </div>
            </SectionReveal>
